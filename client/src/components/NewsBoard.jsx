...
export default function NewsBoard({ category, country, userid, isFavEnabled }) {
      ...
      if (!isFavEnabled) {
        ...
        try {
          const response = await axios.get(
            `http://localhost:3001/like/${userid}`
          );
  ...
  return (
    <div style={{ backgroundColor: "#212F3C" }}>
      <h2 className="text-center mb-4" style={{ color: "white" }}>
        Latest{" "}
        <span
          className="badge bg-danger mb-4"
          style={{ marginTop: "15px", marginBottom: "15px", fontSize:"30px"}}
        >
          News
        </span>
      </h2>
